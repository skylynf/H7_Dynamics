...
for input_file in input_files:
    ...
    output_name = excel_path.stem + "logwfd"  # Use the file name (without extension)
    ...
    for grp, sub in df["wave_host"].groupby("group"):
        ax.plot(sub["time"], sub["distance"], lw=2, color=palette_host[grp], label=grp)
        ax.fill_between(sub["time"], sub["low"], sub["high"], color=fill_host[grp], alpha=0.5, linewidth=0)
    ax.set_xlim(2000, 2025)
    ax.set_yscale("log")  # Set log scale for wavefront distance
    ax.set_ylim(5e3, 2e4)
    ...
    for grp, sub in df["wave_wild"].groupby("group"):
        ax.plot(sub["time"], sub["distance"], lw=2, color=palette_wild[grp], label=grp)
        ax.fill_between(sub["time"], sub["low"], sub["high"], color=fill_wild[grp], alpha=0.5, linewidth=0)
    ax.set_xlim(2000, 2025)
    ax.set_yscale("log")  # Set log scale for wavefront distance
    ax.set_ylim(5e3, 2e4)
    ax.set_xticks(range(2000, 2026, 5))
    ...
    for grp, sub in df["diff_host_weighted"].groupby("group"):
        ax.plot(sub["time"], sub["diffusion_coefficient"], lw=2, color=palette_host[grp], label=grp)
        ax.fill_between(sub["time"], sub["low"], sub["high"], color=fill_host[grp], alpha=0.5, linewidth=0)
    ax.set_xlim(2000, 2025)
    ax.set_yscale("log")
    ax.set_ylim(1e2, 3e7)
    ...
    for grp, sub in df["diff_wild_weighted"].groupby("group"):
        ax.plot(sub["time"], sub["diffusion_coefficient"], lw=2, color=palette_wild[grp], label=grp)
        ax.fill_between(sub["time"], sub["low"], sub["high"], color=fill_wild[grp], alpha=0.5, linewidth=0)
    ax.set_xlim(2000, 2025)
    ax.set_yscale("log")
    ax.set_ylim(1e2, 3e7)
