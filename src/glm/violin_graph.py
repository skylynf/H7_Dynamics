...
def create_combined_plot(activated_coeffs, output_file):
    ...
    ax.set_xlabel('')
    ax.set_ylabel('Coefficient Value', fontsize=18, color='black')
    ...
    ax_bar.set_ylabel('Activation Rate (%)', fontsize=18, color='black')
    ax_bar.set_ylim(0, 105)
    ax_bar.grid(True, axis='y', linestyle='--', alpha=0.3)
    ax_bar.tick_params(axis='x', rotation=45, colors='black', labelsize=16)
    ax_bar.tick_params(axis='y', colors='black', labelsize=18)
    
    plt.setp(ax_bar.get_xticklabels(), ha='right', rotation_mode='anchor')
