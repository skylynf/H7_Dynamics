mccExtractions = function(mcc_tre, mostRecentSamplingDatum) {
  ...
  mcc_tab = mcc_tab[mcc_tab[, "startYear"] >= 2000 & mcc_tab[, "endYear"] <= 2024, ]
  
  mcc_tab = mcc_tab[order(mcc_tab[, "startYear"], decreasing = FALSE), ]
  mcc_tab1 = mcc_tab[1, ]
  mcc_tab2 = mcc_tab[2:dim(mcc_tab)[1], ]
  mcc_tab2 = mcc_tab2[order(mcc_tab2[, "endYear"], decreasing = FALSE), ]
