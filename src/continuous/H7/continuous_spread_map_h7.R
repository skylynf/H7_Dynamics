...
mcc_tre = readAnnotatedNexus("mcc.tree")
...
plot(world, add = TRUE, col = "white", border = "gray30", lwd = 0.1)
...
mcc_tab_ordered <- mcc_tab[order(mcc_tab[,"endYear"], decreasing = FALSE), ]

for (i in 1:dim(mcc_tab_ordered)[1]) {
  curvedarrow(cbind(mcc_tab_ordered[i,"startLon"], mcc_tab_ordered[i,"startLat"]),
              cbind(mcc_tab_ordered[i,"endLon"], mcc_tab_ordered[i,"endLat"]),
              arr.length = 0, arr.width = 0, lwd = 0.2, lty = 1,
              lcol = "gray10", arr.col = NA, curve = 0.1, endhead = FALSE)
}

for (i in dim(mcc_tab_ordered)[1]:1) {
  if (i == 1) {
    points(mcc_tab_ordered[i, "startLon"], mcc_tab_ordered[i, "startLat"],
           pch = 16, col = colour_scale[1], cex = 0.8)
    points(mcc_tab_ordered[i, "startLon"], mcc_tab_ordered[i, "startLat"],
           pch = 1, col = "gray10", cex = 0.8)
  }
  points(mcc_tab_ordered[i, "endLon"], mcc_tab_ordered[i, "endLat"],
